...
export default class Player {
  ...
  constructor(options) {
    this.options = Object.assign(
      {
        port: Player.IS_HTTPS ? '18001' : '18000',
  ...
  connect () {
    return new Promise((resolve, reject) => {
      const CHECK_CMD = {
        comment: { commenttype: 'checkapp', context: 'ideoplayer:' }
      };
      ...
      this.socket = new WebSocket(
        `${Player.IS_HTTPS ? 'wss' : 'ws'}://127.0.0.1:${this.options.port
        }/WebS_Js`
      );
      this.socket.onmessage = checkIsExist;
      this.socket.onopen = () => {
  ...
  run (cmd) {
    ...
    const _run = () => {
      ...
      // const playUrl = url ? url.split(';')[1] : '';
      // window.open("ideoplayer://" + playUrl); // ？是否因此出现了老弹窗，删掉是否会影响现弹窗
    };
    if (!this.isExist)
      this.connect()
        .then(_run)
